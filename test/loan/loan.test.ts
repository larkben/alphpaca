...
import { debug } from "console";
  ...
  describe("loanfi", () => {
    ...
    test('loan (loanfi)', async () => {
        ...
        loanOne = await CreateLoanService(creator, loanFactoryTemplate, ids[0], 20000, ALPH_TOKEN_ID, 10000000000000000000, 800, 3000)
        ...
        await AcceptLoanService(creator, loanFactoryTemplate, hexString, ids[0], 20000, 1738084367000)

        calculatedAmount = await CalculateLoanAssets(nodeProvider, contractAddress, 1751130767000)

        await PayLoanService(creator, loanFactoryTemplate, hexString, ids[0], calculatedAmount, 1751130767000)
      })
  });