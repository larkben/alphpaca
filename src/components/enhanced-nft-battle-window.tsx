    ...
    <div className="container mx-auto p-4 max-w-4xl">
      <Card className="mb-8">
        <CardContent className="p-6">
          <div className="relative h-96 mb-8 bg-cover bg-center rounded-lg overflow-hidden" style={{ backgroundImage: "url('/placeholder.svg?height=400&width=800')" }}>
            <div className="absolute inset-0 bg-black bg-opacity-30"></div>
            <div className="relative z-10 flex justify-between items-end h-full p-8">
              <div className="text-center transform translate-y-4">
                ...
                />
                <div className="font-bold text-white text-lg">Your NFT</div>
                <div className="text-sm text-white bg-gray-50/80 rounded px-2 py-1 mt-1 dark:bg-gray-900/80">HP: {playerHealth}/100</div>
              </div>
              <div className="text-center transform translate-y-[-2rem] translate-x-[-2rem] scale-90">
                <img
                  src="/placeholder.svg?height=200&width=200"
                  alt="Enemy"
                  className="w-40 h-40 object-cover rounded-lg mb-2 border-4 border-red-500 dark:border-red-900"
                />
                <div className="font-bold text-white text-lg">Enemy</div>
                <div className="text-sm text-white bg-gray-50/80 rounded px-2 py-1 mt-1 dark:bg-gray-50/80">HP: {enemyHealth}/100</div>
              </div>
            </div>
          </div>
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <ScrollArea className="h-48 rounded border border-gray-200 p-2 dark:border-gray-800">
                {battleLog.map((log, index) => (
                  <div key={index} className="mb-1">
                    <span className="font-semibold">Turn {log.turn}:</span>{""}
                    <span className={log.attacker === "Player" ? "text-gray-900 dark:text-gray-50" : "text-red-500 dark:text-red-900"}>
                      {log.attacker}
                    </span>{""}
                    used {log.move} for {Math.abs(log.damage)} {log.damage >= 0 ? "damage" : "healing"}
                  </div>
                ))}
              </ScrollArea>
            </CardContent>
